...
export class RouterService {
  ...
  private async findMultiHopRoutes(
    ...
    for (const intermediate of INTERMEDIATE_TOKENS) {
      ...
      if (
        intermediate.contract === tokenIn.contract ||
      ...
      for (const pool1 of firstHopPools) {
        for (const pool2 of secondHopPools) {
          try {
            ...
            const quote = await this.getMultiHopQuote([pool1, pool2], amountIn)
  ...
  private async getMultiHopQuote(
    ...
    // Build path without duplicating intermediate tokens
    const path: string[] = [pools[0].tokenA.contract]
    for (const pool of pools) {
      path.push(pool.tokenB.contract)
    }
